...
//----Here, we encrypt Timestamp and Nonce, and store in EncryptionResult.bin----//
	...
	for(i=0;i<num1;i++)
	{
		...
		{
			printf("%02x",(unsigned char)block[k]);
		}
...
//------------------------------------End---------------------------------------//
	...
	cout << endl<<"Please type the message you want to encrypt below." << endl;
	...
	while(block[0]!='\n')
	{
		...
		//output encryption result after encryption
		for(j=0;j<buffSize+1;j++)
		{
...
//	char *HASH = new char[fileSizeForHash];

	// Each time, read buffSize bytes ciphertext, until read to the end of EncryptionResult.bin
	// i stands for how many bytes we have read
	for (int i = 0; i != fileSizeForHash; i += (buffSize+1)) {
		fread(hashBlock, sizeof(char), buffSize+1, toReadEncryptionResult);
		//Output data read from encryptionResult in hexdecimal mode
		for(j=0;j<buffSize;j++)
		{
			printf("%02x",(unsigned char)hashBlock[j]);
		}
		if(i==0)
		{
	...
	cout<<endl;
	cout<<"here is what we read from binary file for hash"<<endl;
