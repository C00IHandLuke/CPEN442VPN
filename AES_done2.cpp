...
//---------------------------Get Timestamp and Nonce here---------------------//
  ...
  strong_bigdig(&rng,64,2,x); //x is nonce in binary
  //Store x as a decimal string into mip->IOBUFF
  cotstr(x,mip->IOBUFF); // convert x to string 
...
//------------------------------------End---------------------------------------//
	...
	while(block[0]!='\n')
	{
		...
		//Store encryption result in binary mode
		fwrite(block, sizeof(char), buffSize+1, toStoreEncryptionResult);
		printf("\nEncryption text= ");//prints encrypted data
		for (i=0;i<buffSize;i++) printf("%02x",(unsigned char)block[i]);
			printf("\n");
	...
	//	FILE *fp;
		...
		//shs_init(&sh);
		//while((ch=fgetc(fp))!=EOF) shs_process(&sh,ch); // fp=private key of signer 
		//shs_hash(&sh,h);
...
<<<<<<< HEAD
	//@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
	FILE *toReadEncryption = NULL;

	//Read in binary mode
	fopen_s(&toReadEncryption, "EncryptionResult.bin", "rb");
	if (!toReadEncryption) {
		cerr << "Can't open encrypted file to read." << endl;
		exit(EXIT_FAILURE);
	}
	...
	//Compute the length of encryption results
	fseek(toReadEncryption, 0, SEEK_END);
	int fileSize1 = ftell(toReadEncryption);
	fseek(toReadEncryption, 0, SEEK_SET);
	printf("\n%d \n",fileSize1);

	char *hashblock=NULL;
	hashblock = new char [fileSize1];
	cout << hashblock; // the gibberish lien is from here
	char *temp=new char[fileSize1+1];
	sha sh;
	...
	// i stands for how many bytes we have read
		...
		fread(temp, sizeof(char), fileSize1, toReadEncryption);
		temp[fileSize]='\0';
		printf("abc");

		for(i=0;i<fileSize1+1;i++)
		...
		printf("test");
/*
		//this code will convert string of integer to word
...
*/		
		...
		system("pause"); ////2222222222222222222222222222
	
	//shs_hash(&sh,hashblock);
	cout << endl;
	printf("aagasdsadadsada");
	// close toReadEncryptionResult
	fclose(toReadEncryptionResult);
	// release encryptBlock
	delete[] temp;
